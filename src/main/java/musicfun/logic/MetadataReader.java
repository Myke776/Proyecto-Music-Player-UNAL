...
/**
 * Lee metadatos de archivos de audio usando JAudiotagger
 */
public class MetadataReader {
	...
	public SongModel readMetadata(File file) {
		...
		try {
			...
			song.setTitle(title.isBlank() ? fileName : title);
			...
			long duration = audioFile.getAudioHeader().getTrackLength() * 1000L;
	...
	public boolean updateMetadata(SongModel song, File audioFile) {
		try {
			// En una implementación real, usarías JAudiotagger para escribir metadatos
	...
	/**
	 * Verifica si un archivo tiene metadatos legibles
	 */
